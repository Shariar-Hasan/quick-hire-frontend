...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!job) {
    return {
      ...
      description: 'View job details and apply on QuickHire.',
    }
  }

  const company = job.company?.name ?? 'QuickHire'
  const location = job.location?.name ?? 'Remote'
  const title = `${job.title} at ${company} — QuickHire`
  const description = `${job.title} · ${company} · ${location}. Apply now on QuickHire.`
